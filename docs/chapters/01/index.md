...
```mermaid
graph TD
    A[Skin] --> B[Subcutaneous Tissue]
    B --> C[SMAS / Superficial Musculoaponeurotic System]
    C --> D[Parotid-Masseteric Fascia]
    D --> E[Deep Cervical Fascia]
    E --> F1[Superficial Layer - Investing]
    E --> F2[Middle Layer - Pretracheal / Visceral]
    E --> F3[Deep Layer - Prevertebral]
```
...
```mermaid
graph LR
    A[Odontogenic Source] --> B[Primary Space]
    B --> C[Submandibular]
    B --> D[Sublingual]
    B --> E[Submental]
    C --> F[Lateral Pharyngeal]
    D --> F
    F --> G[Retropharyngeal]
    G --> H[Mediastinum]
    F --> I[Cavernous Sinus via pterygoid plexus]
```
